...
import { TVSHOW_DETAIL } from "../redux/constant";
import { CircularProgress } from "@mui/material";
import DetailReviews from "../component/DetailReviews";

export default function TVShowDetail() {
  const selector = useSelector((state) => state.TVShowDetails.tvShowDetail);
              ...
              <Grid item lg={12} container>
                ...
                <Grid item lg={9} md={8} sm={6} xs={12} my={"auto"}>
                  <Typography variant="h3" fontWeight="bold">
                    {selector.title || selector.name} ({fullYear.getFullYear()})
                  </Typography>
                  <Typography variant="subtitle">
                    {selector.release_date}(US)
                    {genres ? (
                      <Box component="span">
                        {genres.map((genres) => {
                          return (
                  ...
                  <Box>
                    <Box sx={{ display: "flex", my: 3 }}>
                      <Box mt={3}>
                        <PercentageProgress mRating={selector.vote_average} />
                      </Box>
                      <Typography>
                        User <br /> Score
                      </Typography>
                      <Avatar sx={{ bgcolor: "#002244", mx: 1 }}>
              ...
              <Grid item lg={12} container>
                ...
                <Grid item lg={3} md={3} sm={4} xs={12} p={3}>
                  ...
                  <Box mt={3}>
                    ...
                    <Typography variant="subtitle">{selector.type}</Typography>
                  </Box>
                </Grid>
              </Grid>
            </Container>
