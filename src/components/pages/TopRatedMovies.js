...
import { TOPRATED_MOVIES } from '../redux/constant'
import LoadMore from '../component/LoadMore'
import MainSkeleton from '../component/MainSkeleton'

export default function TopRatedMovies() {
    const dispatch = useDispatch();
    const selector = useSelector((state) => state.Movie.topRatedMovies)
    useEffect(() => {
        dispatch(
            {
    ...
    return (
        <Box>
            <Container maxWidth="xl" sx={{ mt: 6 }}>
                <Typography variant='h4' sx={{ fontWeight: "bold" }}>Top Rated</Typography>
